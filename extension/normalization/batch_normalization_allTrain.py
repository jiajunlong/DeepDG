...
__all__ = ['BatchNormAllTrain']


class BatchNormAllTrain(nn.Module):
    def __init__(self, num_features, dim=4, eps=1e-5, momentum=0.1, affine=True,
                 *args, **kwargs):
        """"""
        super(BatchNormAllTrain, self).__init__()
    ...
    def forward(self, input: torch.Tensor):
        ...
        output = F.batch_norm(input, self.running_mean, self.running_var, training=True, momentum=self.momentum,
                              eps=self.eps)
...
if __name__ == '__main__':
    GBN = BatchNormAllTrain(4, momentum=0.1, affine=True)
    print(GBN)
    # print(GBN.weight)
    # print(GBN.bias)
    x = torch.randn(16, 4, 1, 1) * 2 + 1
