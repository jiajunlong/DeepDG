...
__all__ = ['dbn_debug', 'DBN_debug']


class DBN_debug_Single(torch.nn.Module):
    def __init__(self, num_features, dim=4, eps=1e-3, momentum=0.1, affine=True,
                 *args, **kwargs):
        super(DBN_debug_Single, self).__init__()
        # assert dim == 4, 'DBN_debug is not support 2D'
        self.eps = eps
...
class DBN_debug(torch.nn.Module):
    def __init__(self, num_features, num_channels=16, dim=4, eps=1e-3, momentum=0.1, affine=True,
        ...
        self.DBN_debug_Groups = torch.nn.ModuleList(
            [DBN_debug_Single(num_features=self.num_channels, eps=eps, momentum=momentum) for _ in
             range(self.num_groups - 1)]
        )
        num_channels_last = self.num_features - self.num_channels * (self.num_groups - 1)
        self.DBN_debug_Groups.append(DBN_debug_Single(num_features=num_channels_last, eps=eps, momentum=momentum))
        for i in range(self.num_groups):
            self.projections.append(self.DBN_debug_Groups[i].maintain_projection)

        print('DBN_debug by ZCA-------m_perGroup:' + str(self.num_channels) + '---nGroup:' + str(
    ...
    def forward(self, X: torch.Tensor):
        ...
        X_hat_splits = []
        for i in range(self.num_groups):
            X_hat_tmp = self.DBN_debug_Groups[i](X_splits[i])
...
if __name__ == '__main__':
    ItN = DBN_debug(8, num_channels=2, momentum=1, affine=False)
    print(ItN)
    ItN.train()
