...
__all__ = ['qrWhitening_debug', 'QRWhitening_debug']


class QRWhitening_debug_Single(torch.nn.Module):
    def __init__(self, num_features, dim=4, eps=1e-3, momentum=0.1, affine=True,
                 *args, **kwargs):
        super(QRWhitening_debug_Single, self).__init__()
        # assert dim == 4, 'QRWhitening_debug is not support 2D'
        self.eps = eps
    ...
    def forward(self, X: torch.Tensor):
        ...
        if self.training:
            ...
            self.maintain_projection.append(wm.data)
...
class QRWhitening_debug(torch.nn.Module):
    def __init__(self, num_features, num_channels=16, dim=4, eps=1e-3, momentum=0.1, affine=True,
        ...
        self.QRWhitening_debug_Groups = torch.nn.ModuleList(
            [QRWhitening_debug_Single(num_features=self.num_channels, eps=eps, momentum=momentum) for _ in
             range(self.num_groups - 1)]
        )
        num_channels_last = self.num_features - self.num_channels * (self.num_groups - 1)
        self.QRWhitening_debug_Groups.append(
            QRWhitening_debug_Single(num_features=num_channels_last, eps=eps, momentum=momentum))
        for i in range(self.num_groups):
            self.projections.append(self.QRWhitening_debug_Groups[i].maintain_projection)

        print('QRWhitening_debug-------m_perGroup:' + str(self.num_channels) + '---nGroup:' + str(self.num_groups))
        self.affine = affine
        self.dim = dim
        shape = [1] * dim
        shape[1] = self.num_features
    ...
    def forward(self, X: torch.Tensor):
        ...
        X_hat_splits = []
        for i in range(self.num_groups):
            X_hat_tmp = self.QRWhitening_debug_Groups[i](X_splits[i])
...
if __name__ == '__main__':
    ItN = QRWhitening_debug(8, num_channels=8, momentum=1, affine=False)
    print(ItN)
    ItN.train()
