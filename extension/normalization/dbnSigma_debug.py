...
__all__ = ['dbnSigma_debug', 'DBNSigma_debug']


class DBNSigma_debug_Single(torch.nn.Module):
    def __init__(self, num_features, dim=4, eps=1e-3, momentum=0.1, affine=True,
                 *args, **kwargs):
        super(DBNSigma_debug_Single, self).__init__()
        # assert dim == 4, 'DBNSigma_debug is not support 2D'
        self.eps = eps
...
class DBNSigma_debug(torch.nn.Module):
    def __init__(self, num_features, num_channels=16, dim=4, eps=1e-3, momentum=0.1, affine=True,
        ...
        self.DBNSigma_debug_Groups = torch.nn.ModuleList(
            [DBNSigma_debug_Single(num_features=self.num_channels, eps=eps, momentum=momentum) for _ in
             range(self.num_groups - 1)]
        )
        num_channels_last = self.num_features - self.num_channels * (self.num_groups - 1)
        self.DBNSigma_debug_Groups.append(
            DBNSigma_debug_Single(num_features=num_channels_last, eps=eps, momentum=momentum))

        for i in range(self.num_groups):
            self.projections.append(self.DBNSigma_debug_Groups[i].maintain_projection)
        print('DBNSigma_debug by ZCA-------m_perGroup:' + str(self.num_channels) + '---nGroup:' + str(
    ...
    def forward(self, X: torch.Tensor):
        ...
        X_hat_splits = []
        for i in range(self.num_groups):
            X_hat_tmp = self.DBNSigma_debug_Groups[i](X_splits[i])
...
if __name__ == '__main__':
    ItN = DBNSigma_debug(8, num_channels=3, momentum=1, affine=False)
    print(ItN)
    ItN.train()
