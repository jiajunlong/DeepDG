...
class QRWhiteningSigma(torch.nn.Module):
    def __init__(self, num_features, num_channels=16, dim=4, eps=1e-3, momentum=0.1, affine=True,
        ...
        self.QRWhiteningSigma_Groups = torch.nn.ModuleList(
            [QRWhiteningSigma_Single(num_features=self.num_channels, eps=eps, momentum=momentum) for _ in
             range(self.num_groups - 1)]
        )
        num_channels_last = self.num_features - self.num_channels * (self.num_groups - 1)
        self.QRWhiteningSigma_Groups.append(
            QRWhiteningSigma_Single(num_features=num_channels_last, eps=eps, momentum=momentum))
