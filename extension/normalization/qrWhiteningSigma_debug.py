...
__all__ = ['qrWhiteningSigma_debug', 'QRWhiteningSigma_debug']


class QRWhiteningSigma_debug_Single(torch.nn.Module):
    def __init__(self, num_features, dim=4, eps=1e-3, momentum=0.1, affine=True,
                 *args, **kwargs):
        super(QRWhiteningSigma_debug_Single, self).__init__()
        # assert dim == 4, 'QRWhiteningSigma_debug is not support 2D'
        self.eps = eps
    ...
    def forward(self, X: torch.Tensor):
        ...
        if self.training:
            ...
            sigma = torch.addmm(beta=self.eps, input=torch.eye(self.num_features).to(X), alpha=1. / m, mat1=xc, mat2=xc.transpose(0, 1))
            self.running_projection = (1. - self.momentum) * self.running_projection + self.momentum * sigma.data
            self.maintain_projection.append(sigma.data)
        else:
...
class QRWhiteningSigma_debug(torch.nn.Module):
    def __init__(self, num_features, num_channels=16, dim=4, eps=1e-3, momentum=0.1, affine=True,
                 *args, **kwargs):
        super(QRWhiteningSigma_debug, self).__init__()
        # assert dim == 4, 'QRWhiteningSigma_debug is not support 2D'
        self.eps = eps
        self.projections = []
        self.momentum = momentum
        ...
        self.QRWhiteningSigma_debug_Groups = torch.nn.ModuleList(
            [QRWhiteningSigma_debug_Single(num_features=self.num_channels, eps=eps, momentum=momentum) for _ in
             range(self.num_groups - 1)]
        )
        num_channels_last = self.num_features - self.num_channels * (self.num_groups - 1)
        self.QRWhiteningSigma_debug_Groups.append(
            QRWhiteningSigma_debug_Single(num_features=num_channels_last, eps=eps, momentum=momentum))
        for i in range(self.num_groups):
            self.projections.append(self.QRWhiteningSigma_debug_Groups[i].maintain_projection)
        print('QRWhiteningSigma_debug-------m_perGroup:' + str(self.num_channels) + '---nGroup:' + str(self.num_groups))
        self.affine = affine
        self.dim = dim
        shape = [1] * dim
        shape[1] = self.num_features
    ...
    def forward(self, X: torch.Tensor):
        ...
        X_hat_splits = []
        for i in range(self.num_groups):
            X_hat_tmp = self.QRWhiteningSigma_debug_Groups[i](X_splits[i])
...
if __name__ == '__main__':
    ItN = QRWhiteningSigma_debug(8, num_channels=3, momentum=1, affine=False)
    print(ItN)
    ItN.train()
