...
class InstanceGroupItN_NLP(nn.Module):
    def __init__(self, num_features, num_groups=4, T=5, num_channels=0, eps=1e-5, momentum=0.1, affine=True, mode=0,
                 *args, **kwargs):
        super(InstanceGroupItN_NLP, self).__init__()
        #input batchsize x tokenNumber x num_features(embeddingDim)
       ...
       # print('InstanceGroupItN_NLP --- num_groups=', self.num_groups, '--T=', self.T)
        if self.affine:
...
if __name__ == '__main__':
    ...
    dbn = InstanceGroupItN_NLP(16, 2, num_channels=0, T=5, affine=True, momentum=1.)
    x = torch.randn(4, 2, 16)
    print(dbn)
    #x = torch.Tensor()
    y = dbn(x)
    print('y size:', y.size())
    y = y.view(y.size(0)*y.size(1), dbn.num_groups, y.size(2) // dbn.num_groups)
